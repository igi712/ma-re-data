...
inline inline uint32_t __builtin_bswap32(uint32_t __x) {
	return __x>>24 | __x>>8&0xff00 | __x<<8&0xff0000 | __x<<24;
}
static inline uint16_t __builtin_bswap16(uint16_t __x) {
	return __x<<8 | __x>>8;
}

//#define err(e, fmt, ...) do { fprintf(stderr, "%s:%d: " fmt ": %s\n", __FILE__, __LINE__, __VA_ARGS__, strerror(errno)); exit(e); } while (0)
#define err(e, ...) do { \
...
int main(int argc, char **argv) {
	...
	for (char **a = argv+1; *a; a++) {
		for (char *s = *a; *s; s++) {
			switch (*s) {
			case '-': break;
			case 'k': keynum = _strtoui64(*++a, NULL, 0); break;
	...
	for (;;) {
		...
		if (len >= buf_len) {
			...
			if (!(buf = malloc(buf_len)))
				err(1, "malloc %lu for %u", buf_len, len);
		}
		if (fread(buf, 1, len, in) != len)
			err(1, "short read data");

		uint16_t off = __builtin_bswap16(*(uint16_t *)buf);
		uint16_t pad = __builtin_bswap16(*(uint16_t *)(buf+2));
		if (buf[7]) {
